...
## The ledger — observability as a projection

> It would be very simple to log everything centrally in the whole system
> right? You could get a company wide overview of what was happening.
> — Willem, 2026-08-17

and, clarifying the target while this section was being written:

> Yeah I meant more for llm automation observability than for something like
> footman invoked by humans.

Architecturally yes, and for a better reason than "add a log call":
observability here is a **projection, not an instrumentation project**. Three
properties conspire, all already in the design:

- **One execution path means one instrumentation point.** The directive that
  every transport dispatches into the same execute path — argued for
  behavioural consistency — buys observability as a side effect: every call,
  from every transport, passes one chokepoint that already mints a typed
  receipt. The `--json` envelope plus two fields *is* the log record; nothing
  is ever parsed. Compare the industry baseline: an agent per host, an SDK
  per app, regex over text.
- **The DAG is already a trace.** OpenTelemetry's data model — traces, spans,
  parent edges, attributes — is a reinvention of what the scheduler holds: a
  run is a trace, a node a span, `pre=` edges the parentage. The hard part of
  distributed tracing, propagating correlation context across boundaries, is
  trivial here specifically because the fabric owns the envelope on every
  transport.
- **Calls are names, so the log is a ledger of resolutions, not a pile of
  prints.** The company-wide view is *which names resolved, from which store,
  at what cost*. "This build ran 400 times across 60 laptops; 390 were
  byte-identical" is a query — and it is the empirical argument for the fleet
  store, generated by the system it would justify.

A symmetry worth keeping: **the manifest and the ledger are the same schema
pointed in opposite directions** — the reflective view forward (what may be
called) and backward (what was called). One contract, projected into future
and past. And the household register already has the word: the **ledger**,
the book the butler kept of everything that entered and left the house.

Four taxes "simple" hides, one of them not technical at all:

1. **The ledger is the most privileged artifact in the company.** Typed args
   include secrets, and `Secret` redaction is already known-incomplete (H46)
   — incomplete redaction goes from bug to incident the moment receipts
   centralise. And read from the other side, "company-wide overview" and
   "monitoring of what every person ran on their laptop" are the same feature
   described by two different lawyers; in the EU that is works-council
   territory. Same *category* of line as the remote rung crossing local-only:
   the architecture makes it possible; a product decision makes it real.
2. **Metadata and payloads are different tiers** — the skip/value split
   again. Receipt metadata (name, effect, outcome, timing, cache hit) is
   cheap, safe, and carries ~all the company-wide value. Argument and result
   *values* carry all the trouble. Default: metadata always, payloads opt-in
   per task, `Secret`-marked parameters never — enforced in the envelope,
   not left to sink configuration.
3. **Observability and audit are different guarantees; merging them breaks
   both.** The sink must be fail-open — no task ever blocks on the log, so
   loss is possible, so it is *not* an audit trail. High-privilege actions
   may want the opposite: a durable receipt written before the action runs,
   fail-closed. Two features sharing a schema — same discipline as not
   merging the skip and the value.
4. **A central view is only as honest as its provenance.** "No receipt
   without work behind it" holds per-process today; centralised, any laptop
   can pollute the company view unless receipts carry attribution. That is
   attestation arriving early for the *third* time (health classification,
   artifact trust, now the ledger) — by this point it is not early, it is a
   prerequisite queueing politely.

**The caller axis, which flips the defaults.** The clarification changes the
taxes more than the mechanism: receipts gain caller provenance — human at a
TTY, script, or agent session with its principal chain — and the defaults
split along it. For *human* callers the taxes above bind tightly:
metadata-only, local-first, and tax 1's surveillance line in full force. For
*agent* callers they mostly invert: an autonomous caller has no privacy
interest, and its accountability requirement is far stronger — the whole
point of watching automation is that no human watched each call happen. So
the observability/audit split from tax 3 maps onto caller kind: fail-open
metadata for people, fail-closed full-payload receipts for agents' actions.
(The principal chain keeps one foot in tax 1 — an agent acts *on behalf of*
someone — but "what the automation did" is a defensible default in a way
"what the human ran" never is.)

**Effects-level, which is the half nobody has.** Agent observability today
is token-level: LangSmith-style traces and the OTel GenAI conventions record
what the model *said* — prompts, completions, tool-call JSON as text. The
ledger records what the automation *did to the world*: which names resolved,
which actions ran, under which capability, what was denied. Both halves are
needed; only a system that types effects can provide the second, which is
why no agent framework has it.

**The tightening loop.** Grant and ledger together close the least-authority
loop: an agent held capability X and used Y ⊂ X, so next session's grant is
Y. The precedent is exact — CloudTrail feeding IAM Access Analyzer's
generated least-privilege policies — and it converts attenuation from
aspiration to an empirical process: the ledger *tells you what to attenuate*.
This also answers the trifecta section's alarm-fatigue problem from the
thinnest list, partially: gates tuned by observed usage fire less often than
gates tuned by fear.

Dogfooding note: the sink needs no new machinery — it is a *service* in the
fabric (an endpoint), receiving receipts over the document transport, with
the crash breaker and idle rules applying to it like anything else. The
observer is another resident of the house.
...
## Where this note is thinnest

Asked directly ("what is the weakest part of the note?", Willem,
2026-08-17), so recorded directly. A note that records what was rejected
should also record where its own argument is soft; a build thread should
read this section first.

**The headline: the sandbox is load-bearing and unpriced.** The note prices
the *declaration* of authority and never prices its *enforcement* — and five
of the security half's most attractive claims converge on that line item:
"physically cannot touch what it did not declare", Scene 1's denied-open
refusal, confined completers, effect honesty ("sandbox enforcement answers
it locally"), and safe offload of derivations. The only cost accounting
offered is "Bazel does this", which is true and misleading: Bazel is a build
system with an enormous budget that controls its whole execution
environment. The platform reality: Linux has real confinement (namespaces,
Landlock) but kernel-version-fractured; macOS effectively does not offer
unprivileged per-process filesystem confinement to third parties
(`sandbox-exec` deprecated, Seatbelt undocumented); Windows is worse. A
stdlib-only, three-platform lineage claiming "physically cannot" is writing
a check the OS layer cashes on roughly one platform. The enforcement-tier
framing ("declared everywhere, enforced where trust changes") gestures at
this but lets the reader believe the subprocess tier is cheap. It is the
single most expensive artifact in the design.

The runners-up, each the same overclaim pattern at smaller scale:

- **"The verifier problem is solved by construction"** — overstated. Markers
  evaluate *value* predicates; caveats over operations ("read-only") and
  domains ("under $100") regress to macaroons' unsolved position, relocated
  rather than solved. The honest sentence is "solved for value caveats,
  *housed* for the rest" — and the note says the stronger thing first and
  the honest thing three sections later.
- **The trifecta policy's false-positive economics.** "Mechanically
  checkable" is true; usably checkable is not established. Session-level
  taint is coarse — almost any real agent session ingests untrusted content
  within minutes, so "actions now gated" fires nearly always, and a gate
  that always fires trains the human to press y. Alarm fatigue is worse than
  no gate, because it looks like a control. Real taint tracking through an
  LLM's context is unsolved by everyone, this design included.
- **"Arguments in normal form" is load-bearing and undesigned.** The entire
  addressing layer assumes it. Floats, default elision, dict ordering, path
  normalisation, locale, environment leakage — each is a way for two
  machines to compute different names for the same call, invisible until the
  second machine exists. The note designs the normal form's *encoding*
  carefully (JCS vs CBOR) while treating the normal form itself as given:
  polishing the spelling of a word nobody has defined.

Criticisable but not thinnest: the absent adoption story (real, but the
note is a thought experiment, not a product plan) and the too-tidy prior-art
table ("each has exactly one piece" flatters the taxonomy; Cap'n Proto has
at least two). Cosmetic next to the enforcement gap.
...
8. **The argument normal form itself** — the thinnest-section admission,
   promoted to a question because it gates the whole addressing layer and
   nothing else on the list can be built soundly before it.
9. **Where the ledger's line is drawn for human callers.** For agents the
   answer is easy and stated (full receipts, fail-closed for actions); for
   humans, metadata-only is a design call, and whether human invocations
   join a company-wide ledger at all is a product-and-people decision of the
   same kind as the remote rung — the one question in this note whose answer
   involves an HR department.
