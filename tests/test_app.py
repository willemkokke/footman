...
def test_self_install_runs_uv_tool_install_upgrade(project, monkeypatch):
    """One command covers both cases: install when absent, latest when
    present. `--with uv` bundles uv so the handoffs work where PATH has none."""
    import subprocess
    ...
    monkeypatch.setattr(_app, "_find_uv", lambda: "/tools/uv")
    monkeypatch.setattr(_app.subprocess, "run", fake_run)
    assert _app.run(["--self-install"]) == 0
    assert spawned == [
        ["/tools/uv", "tool", "install", "--upgrade", "footman", "--with", "uv"]
    ]


def test_self_install_reports_uvs_own_exit_code(project, monkeypatch):
    import subprocess

    monkeypatch.setattr(_app, "_find_uv", lambda: "/tools/uv")
    monkeypatch.setattr(
        _app.subprocess, "run", lambda cmd: subprocess.CompletedProcess(cmd, 2)
    )
    assert _app.run(["--self-install"]) == 2  # uv already said why


def test_self_install_without_uv_teaches(project, monkeypatch, capsys):
    monkeypatch.setattr(_app, "_find_uv", lambda: None)
    assert _app.run(["--self-install"]) == EX_USAGE
    assert "no uv found" in capsys.readouterr().err


def test_self_install_needs_a_declared_distribution(project):
    """A brand without `dist=` gave footman nothing to install; guessing
    would install someone else's package under the user's command name."""
    from footman.app import App
    from footman.testing import Runner

    result = Runner(App(prog="acme")).invoke("--self-install")
    assert result.exit_code == EX_USAGE
    assert "dist=" in result.stderr


def test_self_install_help_wins_and_spawns_nothing(project, monkeypatch, capsys):
    monkeypatch.setattr(_app, "_find_uv", lambda: "/tools/uv")
    monkeypatch.setattr(
        _app.subprocess, "run", lambda cmd: pytest.fail("must not spawn under --help")
    )
    assert _app.run(["--self-install", "--help"]) == 0
    assert "--self-install" in capsys.readouterr().out


def test_self_install_refuses_a_trailing_task(project, monkeypatch, capsys):
    monkeypatch.setattr(_app, "_find_uv", lambda: "/tools/uv")
    monkeypatch.setattr(
        _app.subprocess, "run", lambda cmd: pytest.fail("must not spawn")
    )
    assert _app.run(["--self-install", "check"]) == EX_USAGE
