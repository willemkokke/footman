...
#
# Read from bun 1.3.14 on Linux and macOS. In-process: no.
...
class Bun(_Tool):
    ...
    def add(
        ...
        **flags: Any,
        ...
        Args:
            analyze: Recursively analyze & install dependencies of files passed as
                arguments (using Bun's bundler).
            ...
            force: Always request the latest versions from the registry & reinstall
                all dependencies.
            ...
            minimum_release_age: Only install packages published at least N seconds
                ago (security feature).
            ...
            os: Override operating system for optional dependencies (e.g., linux,
                darwin, * for all).
        ...
        **flags: Any,
        ...
        Args:
            ...
            banner: Add a banner to the bundled output such as "use client"; for a
                bundle being used with RSCs.
            bytecode: Use a bytecode cache.
            chunk_naming: Customize chunk filenames.
            compile: Generate a standalone Bun executable containing your bundled
                code.
            compile_autoload_bunfig: Enable autoloading of bunfig.toml in standalone
                executable. Defaults on — `compile_autoload_bunfig=off` emits
                `--no-compile-autoload-bunfig`.
            compile_autoload_dotenv: Enable autoloading of .env files in standalone
                executable. Defaults on — `compile_autoload_dotenv=off` emits
            ...
            compile_autoload_tsconfig: Enable autoloading of tsconfig.json at
                runtime in standalone executable. `compile_autoload_tsconfig=off`
                emits `--no-compile-autoload-tsconfig`.
            ...
            windows_hide_console: When using --compile targeting Windows, prevent a
                Command prompt from opening alongside the executable.
    ...
    def install(
        ...
        **flags: Any,
        ...
        Args:
            analyze: Analyze & install all dependencies of files passed as arguments
                recursively (using Bun's bundler).
            backend: Platform-specific optimizations for installing dependencies.
            ca: Provide a Certificate Authority signing certificate.
            cache_dir: Store & load cached data from a specific directory path.
            cafile: The same as `--ca`, but is a file path to the certificate.
    ...
    def run(
        ...
        **flags: Any,
        ...
        Args:
            ...
            dns_result_order: Set the default order of DNS lookup results.
            drop: Remove function calls, e.g. --drop=console removes all console.*
                calls.
            elide_lines: Number of lines of script output shown when using --filter
                . Defaults to `10`.
            env_file: Load environment variables from the specified file(s).
            ...
            preserve_symlinks_main: Preserve symlinks when resolving the main entry
                point.
            print: Evaluate argument as a script and print the result.
            redis_preconnect: Preconnect to $REDIS_URL at startup.
            require: Alias of --preload, for Node.js compatibility.
